...
balls_radius = 12
balls = [(300 + np.random.uniform(-30,30), 400 + 50*i + 0.5*i**2) for i in range(100)]
...
for i, finger in enumerate(fingers):
    finger_shape = pymunk.Circle(fingers[i], fing_radius)
    space.add(fingers[i], finger_shape)

# a few color for drawing balls
colors = [(210,150,50), (30, 120, 200), (180, 90, 150),
          (100, 200, 50), (100, 75, 50), (15, 200, 240),
          (60, 70, 230)]
...
with mp_hands.Hands(
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                for i, finger in enumerate(fingers):
                    ...
                    y = img.shape[0]-int(hand_landmarks.landmark[i].y * img.shape[0])
	...
	# getting the position of balls from physics engine and drawing
        for i, ball in enumerate(balls_body):
            ...
            yb = int(img.shape[0]-ball.position[1])
            cv2.circle(img, (xb, yb), balls_radius, colors[i%len(colors)], -1)
